...
<script>
  ...
  $(document).ready(function(){
	  $("#loadingGif").dialog({
		...
		setTimeout(function(){
			$("#loadingGif").dialog("close");
			model.cocktails = JSON.parse(redirect_uri);
			  
			$.each(model.cocktails, function(key, value){
				$("#cocktailsTable").append(" <tr>"+
								             "<td>"+value.id+"</td>"+
								             "<td>"+value.name+"</td>"+
								           "</tr>");
			})	;
		...
		<div class="row-fluid">
			...
			<div class="span10">
				<table class="table table-bordered">
					...
					<tbody id="cocktailsTable"></tbody>
				</table>
			</div>
		</div>
	</div>

	<div id="loadingGif" title="">
		<canvas id="canvas"></canvas>
		<span><p style='text-align:center;'>Retrieving data..</p></span>
	</div>
